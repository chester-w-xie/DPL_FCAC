"""
-------------------------------File info-------------------------
% - File name: main_Nsynth_augmix_S2M_V2_1.py
% - Description: 在main_Nsynth_augmix_S2M_V2_0.py的基础上，调用2.1的模型
% -
% -
% -
% - Input:
...
from DatasetsManager_Nsynth100 import nsynth_dataset_for_fscil, nsynth_dataset_for_fscil_augmix
...
class Trainer(object):
    def __init__(self, args):
        ...
        self.datasets_augmix = nsynth_dataset_for_fscil_augmix(args)
        self.label_per_task = [list(np.array(range(args.base_start_index, args.base_class)))] + [
            list(np.array(range(args.way)) +
                 args.way * task_id + args.base_class)
            for task_id in range(args.tasks)]
        self.base_class_num = args.base_class - args.base_start_index
        self.args.num_class = self.args.num_class - args.base_start_index
    ...
    def meta_training(self, current_session=0, current_trial=1):
        ...
        session_class = self.args.base_class - args.base_start_index + self.args.way * current_session
    ...
    def validation(self, dataset):
        ...
        session_class = self.args.base_class - args.base_start_index
    ...
    def evaluate(self, current_session, current_trial, trained_model):
        ...
        for i, batch in enumerate(test_loader):
            ...
            base_proto = eval_model.fc.weight[:(self.args.base_class - args.base_start_index), :].detach()
            proto = proto.unsqueeze(0).unsqueeze(0)
...
def setup_parser():
    parser = argparse.ArgumentParser(description='Meta-train for nsynth')

    # about dataset and network
    parser.add_argument('-project', type=str, default='Meta-train for nsynth_a.k.a-DPL')
    parser.add_argument('-dataset', type=str, default='nsynth-100',
                        choices=['nsynth-100', 'nsynth-200', 'nsynth-300', 'nsynth-400'])
